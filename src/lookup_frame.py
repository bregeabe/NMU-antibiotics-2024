...
class LookupFrame:
    ...
    def lookup(self):
        ...
        for patient in patients:
            ...
            temp_name = customtkinter.CTkLabel(temp_frame, text=patient[2], font=customtkinter.CTkFont(size=16), width=200)
            temp_dob = customtkinter.CTkLabel(temp_frame, text=patient[3], font=customtkinter.CTkFont(size=16), width=50)
            temp_sex = customtkinter.CTkLabel(temp_frame, text=patient[4], font=customtkinter.CTkFont(size=16), width=50)
            temp_mrn = customtkinter.CTkLabel(temp_frame, text=patient[1], font=customtkinter.CTkFont(size=16), width=50)
            temp_spec_req = customtkinter.CTkButton(temp_frame, text="Specimen Req", command=self.get_spec_req, font=customtkinter.CTkFont(size=16), width=125)
            temp_work_card = customtkinter.CTkButton(temp_frame, text="Work Card", command=self.get_work_card, font=customtkinter.CTkFont(size=16), width=100)
            temp_name.grid(column=0, row=0)
            temp_dob.grid(column=1, row=0)
            temp_sex.grid(column=2, row=0)
            temp_mrn.grid(column=3, row=0)
            temp_spec_req.grid(column=4, row=0)
            temp_work_card.grid(column=5, row=0)
